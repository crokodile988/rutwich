...
async function loadRtcConfiguration(force = false) {
  if (rtcConfigurationPromise && !force) {
    return rtcConfigurationPromise;
  }

  rtcConfigurationPromise = requestJson("/api/rtc-config")
    .then((data) => {
    ...
    .catch((error) => {
      console.warn("Failed to load RTC config, using fallback STUN only:", error);
      return rtcConfiguration;
    });
...
async function fetchRoomInfo(roomId) {
  ...
  const room = await requestJson(`/api/rooms/${encodeURIComponent(normalizedRoomId)}`);
  applyRoomInfo(room);
  return room;
}
...
async function connectViewer() {
  try {
    ...
    viewerState.roomId = normalizeRoomId(roomId);
...
async function connectViewer() {
  try {
    ...
    if (typeof error?.message === "string" && error.message.includes("Room not found.")) {
      ...
      setOverlayBadge("Комната не найдена");
      setViewerMessage("Комната не найдена. Пусть стример создаст новую и пришлёт свежую ссылку.", "error");
    } else {
      setViewerMessage(error.message || "Не удалось подключиться к комнате.", "error");
    }
  } finally {
...
loadLiveRooms();
const autoConnectRoomId = viewerState.roomId;
viewerState.roomId = "";
window.setTimeout(() => {
  if (!autoConnectRoomId) {
    return;
  }

  viewerState.roomId = autoConnectRoomId;
  syncRoomInputs();
  connectViewer();
}, 0);
